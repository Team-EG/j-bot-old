...
class Help(commands.Cog):
    ...
    async def 도움(self, ctx):
        ...
        embed.add_field(name="도움", value="명령어 리스트를 보여줍니다.")
        embed.add_field(name='네이버검색 [검색어]', value='초록창에 검색어를 대신 입력해줍니다.', inline=False)
        embed.add_field(name='구글검색 [검색어]', value='구글 검색창에 검색어를 대신 입력해줍니다.')
        embed.add_field(name='나무위키 [검색어]', value='나무위키 검색창에 검색어를 대신 입력해줍니다.', inline=False)
        embed.add_field(name='유튜브검색 [검색어]', value='유튜브 검색창에 검색어를 대신 입력해줍니다.')
        embed.add_field(name='레벨', value='자신의 채팅 레벨을 보여줍니다.', inline=False)
        embed.add_field(name='정보', value='봇이 있는 서버의 수와 봇과 함꼐 있는 유저의 수를 보여줍니다.')
        embed.add_field(name="유저정보 [유저_맨션]", value="맨션한 유저의 정보를 봅니다. 유저를 맨션하지 않는 경우 자신의 정보를 보여줍니다.", inline=False)
        embed.add_field(name='크레딧', value='이 봇을 만들면서 사용하거나 참고한 것들의 리스트가 적힌 사이트 주소를 출력합니다.')
        embed.add_field(name='대화도움', value='대화봇 기능에 대한 도움말을 출력합니다.', inline=False)
        embed.add_field(name='뮤직도움', value='뮤직 기능에 대한 도움말을 출력합니다.')
        embed.add_field(name="관리자도움", value="관리자 전용 명령어 리스트를 DM으로 보냅니다.- 서버 관리자 이상만 사용 가능", inline=False)
        embed.add_field(name='KSP도움', value='KSP LMP 멀티 서버 관련 명령어 리스트를 출력합니다.')
    ...
    async def 뮤직도움(self, ctx):
        embed = discord.Embed(title='뮤직 명령어 리스트', description='!꼭 프리픽스를 앞에 붙히세요!', colour=discord.Color.red())
        embed.add_field(name="뮤직도움", value="명령어 리스트를 보여줍니다.")
        embed.add_field(name='들어와', value='봇이 보이스 서버에 들어오게 합니다.', inline=False)
        embed.add_field(name="나가", value="봇이 보이스 서버에서 나가게 합니다.")
        embed.add_field(name="재생 [유튜브_url]", value="유튜브 url 음악을 재생합니다.", inline=False)
        ...
        embed.add_field(name='대기 [유튜브_url]', value='유튜브 url 음악을 대기 리스트에 넣습니다.', inline=False)
    ...
    async def 관리자도움(self, ctx):
        ...
        embed.add_field(name='킥 [유저-맨션-혹은-ID]', value='맨션한 유저를 추방합니다.')
        embed.add_field(name='밴 [유저-맨션-혹은-ID]', value='맨션한 유저를 밴합니다.', inline=False)
        embed.add_field(name='프리픽스교체 [프리픽스]', value='프리픽스를 교체합니다.\n주의! - 띄어쓰기가 필요한 경우 "[프리픽스]" (쌍따옴표 붙이기)로 해주세요!')
        embed.add_field(name='정리해 [숫자]', value='숫자만큼의 채팅을 삭제합니다.', inline=False)
        embed.add_field(name='XP리셋 [유저-맨션-혹은-ID]', value='해당 유저의 XP를 초기화합니다.')
        embed.add_field(name='XP입력 [유저-맨션-혹은-ID] [XP값]', value='해당 유저의 XP값을 입력한 숫자로 바꿉니다.', inline=False)
        embed.add_field(name='도배리셋 [유저-맨션-혹은-ID]', value='해당 유저의 도배 기록을 리셋합니다.')
        embed.add_field(name='대화프픽 [프리픽스]', value='대화봇 기능용 프리픽스를 교체합니다.', inline=False)
        embed.add_field(name='대화DB [동기화 또는 서버전용]', value='모든 서버와 동기화된 대화 데이터베이스 사용 여부를 설정합니다.')
        embed.add_field(name='레벨기능 [사용 또는 미사용]', value='레벨 기능 사용 여부를 설정합니다.', inline=False)
        embed.add_field(name='도배방지 [사용 또는 미사용]', value='도배 방지 기능 사용 여부를 설정합니다.')
        embed.add_field(name='인사말 [환영인사말]', value='새 유저가 들어오면 환영 채널에 출력할 말을 설정합니다.', inline=False)
        embed.add_field(name='작별인사 [작별인사말]', value='유저가 나가면 환열 채널에 출력할 말을 설정합니다.')
        embed.add_field(name='DM인사말 [DM환영인사말]', value='새 유저가 들어오면 DM으로 보낼 인사말을 설정합니다.', inline=False)
        embed.add_field(name='환영채널 [채널이름_#없이]', value='환영 채널을 설정합니다.')
        embed.add_field(name='서버설정', value='현재 서버 설정을 출력합니다.', inline=False)
        embed.add_field(name='설정리셋', value='서버 설정을 리셋합니다.')
